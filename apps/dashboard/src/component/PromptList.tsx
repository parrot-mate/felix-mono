...
const highlightText = (text: string, term: string) => {
  ...
  return (
    <span>
      {before}
      <span className="rounded bg-yellow-200 px-1 text-xs font-medium text-slate-800">
        {match}
      </span>
      {after}
    </span>
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <Link
        ...
        className="flex items-center justify-center rounded-full p-1 text-emerald-400 transition-colors hover:bg-emerald-500/10 hover:text-emerald-300"
        title="Run prompt"
      >
...
export const PromptList = ({
  ...
  if (keys.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center rounded-lg border border-slate-700 bg-slate-900 p-6 text-slate-100">
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-lg border border-slate-700 bg-slate-900">
      <div className="border-b border-slate-700 px-4 py-3">
        <Title level={5} className="!mb-0 font-semibold text-slate-100">
          Prompts
        </Title>
      </div>
      <div className="flex-1 overflow-y-auto px-2 py-3">
        <Tree
          ...
          className="[&_.ant-tree-title]:text-slate-100 [&_.ant-tree-switcher]:text-slate-400 [&_.ant-tree-node-selected_.ant-tree-title]:text-primary-400"
        />
      </div>
    </div>
  )
